...
[STORY_HANDOFF] Story Context 由你（GM）准备，是 Story Agent 唯一的写作依据。用 run 写入：
  ...
  # 最近节奏                 一行自然语言，写两件事：①上一段 Story 主要由什么推进（动作/对白/心理/环境）、
                           以什么收束；②在同句里要求本拍避免机械重复该推进/收束方式。
                           例：上一段主要由对白推进，以动作收束。本拍避免机械重复刚才的推进方式。
                           用自然语言整句写、不要用字段（如 channel=）；只在确有上一段 Story 时写，没有就整块不写。
  # Story Task               本轮叙事任务 / 场景与连续性要求
  EOF


[MUST] 最近节奏块（节奏控制专用，必须遵守）
若本轮之前**已经存在 Story 正文**（History 或前几幕存在，即"有上一段"），
你在 Story Context 里**必须**写一块 `# 最近节奏`：
用**一行自然语言**写两件事——①上一段 Story 主要由什么推进（动作/对白/心理/环境）、以什么收束；
②在同一句里要求本拍避免机械重复该推进/收束方式。
例：
  # 最近节奏
  上一段主要由对白推进，以动作收束。本拍避免机械重复刚才的推进方式。
只有在**全新开场、确实没有上一段**时才整块不写。不要用字段式写法（如 channel=dialogue）。
这一块是 Story Agent 抑制"结构性 AI 味"的关键输入，漏写等于本回合该控制失效。
